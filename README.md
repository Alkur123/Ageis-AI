...
**1,001 adversarial samples · 12 harm categories · 97 attack types · Training set: 8,369 labeled samples (384-dim embeddings)**
...
| **Live deployed corpus today** | **8,369** | (eval below ↓) | (eval below ↓) | (eval below ↓) | — | — |

> The 2,416-vector corpus was the eval-frozen build that produced the 99.30%/100%/99.20% number above. The live system has since grown to **8,369 vectors / 384-dim** (verified: `precomputed_embeddings.npy` shape `(8369, 384)`) and is benchmarked separately below against a *harder* adversarial set.
...
**1,000 samples · `backend/eval/redteam_eval_benchmark_1k (1).csv` · run on the live 8,369-vector corpus**
*Result file: [`backend/eval/results/eval1k_results_20260504_013600.json`](backend/eval/results/eval1k_results_20260504_013600.json)*

This is a **deliberately harder** benchmark than the V3 adversarial set. It was built specifically to find the engine's failure surface for production hardening — sophisticated false-positive traps, defensive cybersecurity framing, dual-use knowledge, translation traps, India-specific framing, fictional bypass scaffolds. The 100 SAFE samples here are *adversarially safe* (educational queries about dangerous topics that earlier classifiers consistently over-blocked).
...
| **False Negative Rate** | **9.11%** |
| True Positives (harmful blocked) | **818** |
| True Negatives (safe allowed) | **78** |
| False Positives (safe over-blocked) | **22** |
| False Negatives (harmful missed) | **82** |
...
```
                    PREDICTED
                  HARMFUL   SAFE
ACTUAL  HARMFUL     818      82      Total: 900
ACTUAL  SAFE         22      78      Total: 100
                                     Total: 1,000
```

**Per-category — every harmful category retains 100% precision even on this harder set:**

| Category | Samples | Recall | F1 | FN |
|----------|--------:|-------:|---:|---:|
| **SYSTEM_EXFILTRATION** | 86 | **94.19%** | 97.01% | 5 |
| **SEXUAL** | 84 | **94.05%** | 96.93% | 5 |
| **SELF_HARM_PASSIVE** | 83 | **92.77%** | 96.25% | 6 |
| **ILLEGAL** | 83 | **92.77%** | 96.25% | 6 |
| **LEGAL** | 83 | **92.77%** | 96.25% | 6 |
| **FINANCIAL** | 84 | **91.67%** | 95.65% | 7 |
| **PII** | 84 | **91.67%** | 95.65% | 7 |
| **PROMPT_INJECTION** | 78 | **91.03%** | 95.30% | 7 |
| **MEDICAL** | 73 | **86.30%** | 92.65% | 10 |
| **VIOLENCE** | 79 | **86.08%** | 92.52% | 11 |
| **SELF_HARM** | 83 | **85.54%** | 92.21% | 12 |
| **SAFE** | 100 | 78.00% spec. | — | 22 FP |

**Per-adversarial-type breakdown — the failure surface of the engine:**

| Adversarial Type | Total | Correct | Accuracy | Notes |
|---|--:|--:|--:|---|
| `false_positive_trap` | 53 | 47 | **88.68%** | Educational/wellness queries near harmful topics |
| `defensive_framing` | 36 | 32 | **88.89%** | Cybersecurity defense-side framing |
| `academic_framing` | 48 | 37 | **77.08%** | "For my thesis…" — hardest FP class |
| `dual_use_knowledge` | 7 | 5 | **71.43%** | Information that is both safe and harm-enabling |
| `translation_trap` | 4 | 3 | **75.00%** | Cross-lingual semantic drift |
| `india_specific` | 12 | 8 | **66.67%** | India-context FP traps (lowest accuracy class) |
| `fictional_bypass` | 36 | 33 | **91.67%** | "Write a screenplay where…" |

**Why the numbers drop here vs Benchmark 1:** the V3 set has 130 SAFE samples that are *clearly* safe. The Hard Redteam 1K has 100 SAFE samples that are *adversarially safe* — chemistry of explosives explained for mining engineers, phishing techniques explained for university defense courses. The engine over-blocks on 22 of these. **This is the honest production failure surface.** It is exactly the dataset class that V4 work is targeted at.

**Reading the result correctly:**
- 100% precision across **all 11 harmful categories** holds even on this set — the engine never confuses harm classes
- 90.89% recall on adversarially-framed harm is **far above** any general moderation baseline (60.16% on the easier V3 set)
- The 22% FPR is concentrated in 3 narrow framing classes (academic, india_specific, dual_use) — addressable with targeted training data, not a structural problem
...
| **RAM Footprint** | **~50 MB** |
| **GPU Required** | None |
| **FAISS Index (live)** | **8,369 vectors · 384-dim · `IndexFlatIP`** (eval-locked snapshot: 2,416 vectors → 99.30% on V3 set) |
...
Honest engineering demands honest limitations. These are the genuine, measured failure modes of Chakravyuha V3 today — not marketing softening.

### A. Detection-layer limitations

| # | Limitation | Where it shows up | Severity |
|---|---|---|---|
| L-01 | **Sophisticated false-positive traps** — 22% FPR on the Hard Redteam 1K (Benchmark 4) on adversarially-safe queries (e.g. educational chemistry of explosives, defensive cybersecurity framing) | Benchmark 4 SAFE column, `india_specific` (66.67%), `academic_framing` (77.08%), `dual_use_knowledge` (71.43%) | **High** — the dataset class V4 is targeted at |
| L-02 | **L33tspeak / character-substitution obfuscation** — 5 of 7 V3 false negatives are character substitution (`s3lf-termin4te`) that operate below the embedding tokenizer | V3 SELF_HARM/FINANCIAL FNs | Medium |
| L-03 | **Encoding-wrapper bypass** (pig-latin, base64, ROT13) — 1 V3 PROMPT_INJECTION FN | V3 PROMPT_INJECTION FN | Medium |
| L-04 | **PMLA-specific regulatory keyword gap** — 1 V3 FINANCIAL FN | V3 FIN-073 | Low |
| L-05 | **Translation-trap cross-lingual drift** — 75% accuracy on translation_trap class (3/4 in Benchmark 4) | Benchmark 4 | Low (small class today) |
| L-06 | **Output verifier ceiling** — Ring 7 re-classifies the LLM output post-hoc; an adversary aware of Ring 7 thresholds could craft outputs that pass by margin | Theoretical / red-team | Medium |

### B. Operational / deployment limitations

| # | Limitation | Where it shows up | Severity |
|---|---|---|---|
| L-07 | **Demo runs single-tenant in-memory** — `REDIS_URL` and `POSTGRES_URL` not wired on the free EC2 demo; sessions lost on restart, no real multi-tenancy | Live demo only | High (cost-bound) |
| L-08 | **No streaming LLM mid-generation interrupt** — Ring 7 only fires after the full LLM response | Production architecture | Medium |
| L-09 | **Federated learning is scaffolding** — gradient submission + FedAvg + ε-budget exist, but federated FAISS index update isn't wired to live tenants yet | `engine/federated.py` | Medium |
| L-10 | **MaxMind GeoIP DB not bundled** — falls back to tenant-declared jurisdiction | `engine/jurisdiction.py` | Low |
| L-11 | **Single-region deployment** — `us-east-1` only; no multi-region failover | AWS infra | Medium |
| L-12 | **HTTP-only MCP transport** — no stdio transport for IDE-native agent integration | MCP layer | Low |

### C. Governance scope limitations

| # | Limitation | Where it shows up | Severity |
|---|---|---|---|
| L-13 | **Single-turn governance** — pipeline governs one query at a time; multi-turn drift across a session is tracked via Ring 4 but not modelled as a first-class trajectory | Architecture | **High** (this is what V5 solves) |
| L-14 | **No tool-call governance** — the engine governs queries, not tool invocations issued by an agent | Architecture | **High** (V5) |
| L-15 | **No agent-to-agent communication governance** — when one agent passes a payload to another, Chakravyuha doesn't gate it | Architecture | **High** (V5) |
| L-16 | **No goal-shift detection** — the engine has no notion of an agent's declared goal vs its actual trajectory | Architecture | **High** (V5) |
| L-17 | **No formal third-party security audit** — the system has not yet been pen-tested by an external firm; SOC2 not certified | Compliance | High for enterprise |
| L-18 | **Single-author + AI-pair-programmed** — depth of code review is limited; no second engineer has independently audited the cryptographic vault construction | Engineering process | Medium |

---

## Future Roadmap — How V4 Closes Each Limitation

Every limitation above maps to a specific, scoped V4 work-item. This is the funded build plan.

### Chakravyuha V4 — Hardening Release

| Closes | V4 Work-Item | Approach |
|---|---|---|
| **L-01** | Adversarial-aware retraining round | Expand training corpus from 8,369 → 15,000+ with synthetic generation focused on `academic_framing`, `india_specific`, `dual_use_knowledge` classes. Per-class FPR target ≤ 5%. |
| **L-02 / L-03** | Pre-tokenizer normalizer | Un-l33t + homoglyph normalizer + encoding-wrapper detector before ONNX tokenization. Resolves the 5/7 obfuscation FNs at structure level. |
| **L-04** | India regulatory micro-corpus | 200 PMLA / GST / RBI / FEMA-specific anchors. Closes the regulatory keyword gap. |
| **L-05** | Multilingual upgrade | Swap `all-MiniLM-L6-v2` → `multilingual-e5-large` for Hindi/Hinglish/translation_trap parity. |
| **L-06** | Stability-anchored Ring 7 | Ring 7 re-classifies *plus* paraphrases the output and checks stability — no fixed-margin bypass surface. |
| **L-07** | Production stateful tier | ElastiCache Redis cluster + RDS PostgreSQL Multi-AZ on `ap-south-1` + DR replica on `us-east-1`. Demo upgrades from in-memory to fully stateful. |
| **L-08** | Streaming Ring 7 (token-level) | Mid-generation interrupt at token boundaries via Groq streaming + incremental classifier. Sub-token latency, full output protection. |
| **L-09** | Federated FAISS rollout | Per-tenant gradient → server-side FAISS update with differential-privacy ε budget enforced at write. |
| **L-10** | MaxMind GeoIP bundling | License + bundle in image. Per-request jurisdiction with audit trail. |
| **L-11** | Multi-region failover | Active-active `ap-south-1` + `us-east-1` with Route 53 latency routing + CloudFront origin failover. |
| **L-12** | stdio MCP transport | stdio + HTTP dual transport so IDE agents (Cursor, Cline, Continue) can integrate without network layer. |
| **L-17** | Third-party audit | External pen-test + SOC2 Type II certification + formal cryptographic review of the audit vault. |
| **L-18** | Open-source governance | Public eval reproduction kit + public adversarial benchmark + community plugin contribution path. |

### V4 also adds 4 new regulation plugins
- `pdpa_singapore.py` · `pdpa_thailand.py` · `lgpd_brazil.py` · `popia_south_africa.py` · `nist_ai_rmf.py` (US framework)
- Total → **11 plugins · ~150 clauses**

---

## Next Version — Chakravyuha V5: Governance for AI Agents

> Agents are booming. Single-query governance is yesterday's problem. **Chakravyuha V5 governs the agent.**

The 2026 LLM landscape is shifting from *one query → one response* to *multi-step autonomous agents* — Claude Code, Cursor agents, OpenAI Operators, AutoGPT-class systems, agentic browsers, agentic SQL writers, agentic DevOps. These systems take **dozens of actions per session, invoke tools, write files, send messages, talk to other agents**. Single-turn governance is structurally insufficient for them.

This is what V5 is built for.

### What changes in V5

| Aspect | V3 / V4 (today) | V5 (next) |
|---|---|---|
| **Governance unit** | One query | One **agent session** (multi-turn trajectory) |
| **Decision surface** | ALLOW / BLOCK / SUPPORT / HITL on a string | ALLOW / BLOCK / RETRY / KILL_SESSION on a trajectory |
| **Observable** | Query text | Query · tool calls · file writes · network egress · agent-to-agent msgs |
| **State model** | Session risk decay (Ring 4) | Agent **identity** + behavioural fingerprint + declared-vs-actual goal divergence |
| **Audit unit** | One decision row | One **agent trajectory tree** (rollup over N steps) |
| **Compliance** | EU AI Act Art. 12 (audit) | EU AI Act Art. 14 (human oversight) **+ Art. 26** (deployer obligations) |

### New rings introduced in V5

```mermaid
flowchart LR
    R12["<b>Ring 12 · Trajectory Verifier</b><br/>Goal-drift detection<br/>Plan ↔ execution match"]:::r12
    R13["<b>Ring 13 · Tool-Call Gate</b><br/>Per-invocation governance<br/>Capability allowlist"]:::r13
    R14["<b>Ring 14 · Inter-Agent Bus</b><br/>Agent-to-agent message gate<br/>Cross-agent identity check"]:::r14
    R15["<b>Ring 15 · Behavioural Fingerprint</b><br/>Persistent agent identity<br/>Anomaly detection"]:::r15
    R16["<b>Ring 16 · Plan-of-Record</b><br/>ZK proof of plan→action match<br/>Agentic compliance evidence"]:::r16

    R12 --> R13 --> R14 --> R15 --> R16

    classDef r12 fill:#3b82f6,stroke:#1d4ed8,stroke-width:2px,color:#fff;
    classDef r13 fill:#10b981,stroke:#047857,stroke-width:2px,color:#fff;
    classDef r14 fill:#8b5cf6,stroke:#6d28d9,stroke-width:2px,color:#fff;
    classDef r15 fill:#f59e0b,stroke:#b45309,stroke-width:2px,color:#000;
    classDef r16 fill:#ec4899,stroke:#be185d,stroke-width:2px,color:#fff;
```

### V5 capabilities

- **Agent identity primitive** — every agent has a cryptographically anchored identity. Behavioural fingerprint (tool-use distribution, latency profile, decision cadence) detects compromised or impostored agents.
- **Tool-call governance** — every `function_call` / MCP tool invocation passes through Ring 13 with per-tenant capability allowlist. A finance agent cannot invoke a filesystem tool without explicit permission.
- **Goal-shift detection** — at session start, the agent declares its goal (`"summarise this PDF"`). Ring 12 continuously checks each step against the declared goal; large divergence → HITL.
- **Cross-agent communication audit** — when Agent A passes a payload to Agent B, Ring 14 governs the handoff *and* writes a linked audit entry to both agents' chains.
- **Plan-of-Record proofs** — Ring 16 emits a zero-knowledge proof that the agent's executed plan matches its declared plan, *without* revealing the proprietary plan content. Solves enterprise IP concerns under EU AI Act Article 26.
- **Trajectory rollup audit** — instead of N audit rows for an N-step agent, Ring 11 produces a single rollup tree node + a hash chain over child decisions. Discoverable, mathematically verifiable.

### Why this matters now

EU AI Act Article 14 mandates **meaningful human oversight** over high-risk agentic systems. Article 26 places deployer obligations on anyone running a third-party AI agent in production. **Neither obligation can be satisfied by single-query governance** — they require trajectory-level evidence.

By August 2026, every Indian fintech, healthtech, and enterprise running an agentic LLM workflow will need this layer or risk €30M / 6%-revenue penalties. **Chakravyuha V3 is the foundation. V5 is the layer the entire agentic-AI stack needs.**

### V5 status

| Component | Status |
|---|---|
| Architecture spec | Drafted (`CHAKRAVYUHA_V3_BLUEPRINT.md` § agentic extensions) |
| Ring 12 prototype | Research code, not in `engine/` yet |
| Ring 13 (tool-call gate) | Design phase — depends on stable MCP capability schema |
| Ring 14 / 15 / 16 | Research |
| Funded by | Pre-seed round (in progress) |
