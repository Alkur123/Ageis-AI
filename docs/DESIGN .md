...
[![Pipeline](https://img.shields.io/badge/Pipeline-13--Ring_+_Agentic-ec4899?style=flat-square)]()
[![Theorem](https://img.shields.io/badge/Bounded--Evasion-Proven-8b5cf6?style=flat-square)]()
...
- [Agentic Governance Design (Ring 12 & 13)](#agentic-governance-design)
- [Bounded-Evasion Design](#bounded-evasion-design)
...
| **Determinism around the LLM** | The 13 deterministic rings make the decision. The LLM colors the response. We never let a hallucinating model decide whether to block. |
...
### Decision 1 — 13 deterministic rings (not single LLM-as-judge)
...
- *And for agents:* an LLM judge of a multi-step trajectory is itself bypassable (arXiv:2507.05630) — the safety hot path must be deterministic

**Chosen:** 13 deterministic rings. Rings 0–5 use regex, FAISS, and rule engines — fully deterministic, sub-50ms, paraphrase-stable. Ring 6 is the LLM (called only after the input has been classified ALLOW). Rings 7–11 verify the LLM output, atomic-commit the decision, and persist the audit trail. Rings 12–13 govern an agent's *trajectory* — every step scored deterministically against a signed declaration of intent.
...
- Index is `IndexFlatIP` (exact) not `IVF` because the live corpus (8,627 vectors) is still small enough that exact search beats quantization on both speed and recall
...
```
Training Corpus:
  eval-locked V7 snapshot: 2,416 labeled samples → 99.30% on the V3 set
  live deployed corpus:    8,627 labeled vectors · 12 categories
...
The V3 keystone: governing not a single query but an agent's whole *session* of actions. This is a different design problem — the unit is a trajectory, the ground truth is "did the agent stay within what it declared", and the adversary can hide harm across many individually-innocent steps.

### Decision 13 — Govern the trajectory, not the prompt (Ring 12)

**Rejected:** Re-running the single-query classifier on each agent step.
**Why rejected:** the step "read this file" is benign in isolation; the harm is *reading `/etc/passwd` when the declared goal was "summarise the quarterly PDF"*. Context, not content, is the signal.

**Chosen:** a 5-signal drift score computed at every step against a declared goal/plan:

| Signal | Measures | Weight |
|---|---|---:|
| S1 — cosine drift | step-embedding EMA vs declared-goal embedding | 0.20 |
| S2 — action-class drift | Jaccard distance of action classes vs expected | 0.25 |
| S3 — tool surprise | allowlist / soft-warn / off-list | 0.25 |
| S4 — plan-execution match | cosine vs declared plan steps | 0.15 |
| S5 — paraphrase stability | deterministic in-network re-classification | 0.15 |

`aggregate = (0.20·S1 + 0.25·S2 + 0.25·S3 + 0.15·S4 + 0.15·S5) / 1.5`, WARN at 0.45, KILL at 0.70.

**Why S5 is deterministic, not Groq:** an LLM-judged stability check is itself bypassable and adds a third-party call to the security hot path. The default is a meaning-preserving local transform classified by the ONNX engine (Groq opt-in via `R12_S5_USE_LLM=1`). Every eval is then byte-reproducible offline.

### Decision 14 — Declaration-independent hard-block rings

**Problem:** scope/drift signals are *non-discriminative* under a faithful declaration — if an attacker declares its real (or a plausible cover) toolset, the harmful chain stays "in scope" and produces zero drift. Measured, not assumed: on the generated set, every harmful kill-chain was a subset of a plausible cover.

**Chosen:** rings that fire on trajectory *structure*, regardless of what was declared:
- **Secret→egress flow** — a session that reads a managed secret and then egresses to an *external* destination is hard-blocked. The FP-safe discriminator is internal-vs-external destination (benign agents read secrets too, but egress internally).
- **Provenance / pipeline-poisoning** — a compute step consuming a self-authored artefact.
- **Capability-assembly** — synthesis after ≥N distinct fragment reads.
- **Privilege-escalation / tool-scope** — `sudo`/scope breaches.

This is where the real moat lives: a content/flow detector, not the scope gate.

### Decision 15 — Close the gate behind the agent (Ring 13, Jayadratha)

**Rejected:** Re-evaluating every subsequent step after a kill.
**Chosen:** once Ring 12 emits WARN/KILL, Ring 13 *freezes* the session — all later egress returns `FROZEN` instantly. Named for the warrior who sealed the Chakravyuha behind Abhimanyu: the agent is inside the formation and cannot reinforce its behaviour.

### Decision 16 — Sign the intent (Sandhi)

**Problem:** drift is measured against session state — but an attacker with write access to session memory could move the declared goal.
**Chosen:** an Ed25519 keypair per agent signs `goal + plan + allowed_tools + allowed_classes` at session start. Modification is cryptographically detectable. Multi-pod via the control plane (public key + KMS-wrapped seed; plaintext seed never stored).

### Decision 17 — Herd immunity (Conch) + rotating blindspots (Sequential)

- **Conch (Pāñchajanya):** when one tenant kills a novel attack, a fingerprint broadcasts to all tenants → 24h elevated vigilance. Fails *safe* (no amplification) when the control plane is down — it is an amplifier, not a gate.
- **Sequential Engagement:** static detectors have static blindspots. Each ring carries rotating variant detectors with non-overlapping blindspots; HITL outcomes auto-quarantine/promote them. An adversary who maps detector A meets detector B next session.
- **Vyuha Selector:** a pre-pipeline posture router (a hint, not a gate) that picks the engagement formation from attack signals + PII.

---

## Bounded-Evasion Design

The recession property is **proven**, not just engineered (`docs/BOUNDED_EVASION_THEOREM.md`):

- **Theorem 1 — Dichotomy:** a fully-informed attacker is either caught within a couple of steps, or confined so far sub-band it can never reach harm.
- **Theorem 2 — Duty-cycle:** an attacker that stays hidden can push the boundary at most ⅓ of the time; the rest must be honest (and measured).
- **Proposition 3 — Coverage:** every harmful path is caught in bounded time, caught immediately by a structural hard-block, or falls into one named residual class.

**Design consequence:** a single-checkpoint filter is just the special case where the boundary never tightens — which is exactly why those get worn down. The penetration-depth recession is the design choice that makes the boundary un-patiently-bypassable.
...
### From tamper-*evident* to tamper-*proof* (HMAC + WORM anchor)

A plain hash chain is tamper-*evident* but not tamper-*proof*: an attacker with DB write access could recompute the *whole* chain after editing an entry. Two design additions close this:

1. **HMAC-SHA256 per entry**, keyed by `AUDIT_SIGNING_KEY` — a 32+ byte secret injected from Secrets Manager/KMS and **never stored in MongoDB**. Without the key, a DB-write attacker cannot forge a valid signature, so a recomputed chain fails `verify_chain` closed.
2. **Off-DB head anchor** — the chain head is periodically PUT to an **S3 Object-Lock (WORM)** bucket. `verify_chain_from_db` reads the latest anchor back and fails closed on truncation/rollback. Local `audit_anchors.jsonl` is the demo fallback.

This is what makes the audit vault legally load-bearing: EU AI Act Article 12 wants an audit trail; a regulator wants one a *DB admin cannot quietly rewrite*.
...
### Why a four-package ecosystem (not one monolith)

Adoption surface dictated the split:
- **`aegis-ai-sdk` (PyPI v1.1.0)** — the governance client + `@govern` decorator + trajectory client.
- **`aegis-ring12` (PyPI v0.1.0)** — the standalone agent trajectory verifier, installable with zero backend (research + embedding in agent runtimes).
- **`@aegis.org/sdk` (npm v1.1.0)** — TS/JS parity for Node/edge agents.
- **`@aegis.org/mcp` (npm v0.1.0)** — an **MCP server**: the "universal adapter". Claude Code, Cursor, Codex, and LangGraph route through one Aegis governance brain with almost no glue. Design bet: the governance decision point (PDP) is the backend; each agent gets a thin adapter, not a fork.
...
### 7-tab intelligence console

| Tab | Purpose |
|---|---|
| **RISK** | RiskGauge · DecisionComparator · SessionPanel · RiskGraph |
| **SESSION** | SessionViewPanel — pipeline trace · history · R12 + vault head |
| **EVIDENCE** | SignalBreakdown · IntentGuardSummary · PolicyComposedSummary · EvidencePanel · Ring3 clauses · Timeline |
| **AGENTS** | TrajectoryPanel (R12) · JayadrathaPanel (R13) · SandhiPanel · ConchPanel |
| **MONITOR** | MetricsPanel · HeatmapPanel · AuditLogs |
| **SYSTEM** | StatsPanel · TenantDirectoryPanel · VaultPanel · RegulationsPanel · VyuhaPanel · SequentialPanel · FederatedPanel |
| **HITL** | HitlPanel — queue · stats · breaches · claim/resolve |

The split exists because **operators care about different things at different times**. A debugging engineer wants Evidence; a CISO wants Compliance; an SRE wants System; an agent-platform owner wants Agents; an admin wants HITL. One tab per persona keeps every panel relevant.
...
| 13 deterministic rings | More code than single LLM call | Consistency · no hallucinated citations · sub-20ms |
| Trajectory verifier (R12/R13) | A whole agentic subsystem to maintain | Catches agent goal-drift a single-query filter is structurally blind to |
| Penetration-depth recession | Non-standard, needs a proof | Makes the boundary un-patiently-bypassable (bounded-evasion theorem) |
| Deterministic S5 (not Groq) | Lower ceiling than an LLM judge | Reproducible + no third-party call on the security hot path |
| Ed25519-signed declarations (Sandhi) | Key management per agent | Drift measured against tamper-proof intent, not mutable session state |
...
*Chakravyuha V3 Design Document — Production AI governance infrastructure · Built solo by Jaswanth Alkur · [Back to README](../README.md)*
*Live demo: https://d1qmmfoekk6xmp.cloudfront.net/ · SDKs: [PyPI](https://pypi.org/project/aegis-ai-sdk/) · [npm](https://www.npmjs.com/package/@aegis.org/sdk) · Paper: https://doi.org/10.5281/zenodo.19690403 · Writing: https://ajaswanth.substack.com*
