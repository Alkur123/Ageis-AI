...
[![Functional](https://img.shields.io/badge/Functional_Requirements-FR--80+-6366f1?style=flat-square)]()
[![Non-Functional](https://img.shields.io/badge/Non--Functional-NFR--25+-8b5cf6?style=flat-square)]()
[![Agentic](https://img.shields.io/badge/Agentic-Ring_12_+_13-3b82f6?style=flat-square)]()
[![Accuracy](https://img.shields.io/badge/Accuracy-99.30%25-22c55e?style=flat-square)]()
[![Acceptance](https://img.shields.io/badge/Acceptance_Criteria-14_Scenarios-ec4899?style=flat-square)]()
...
- 13-ring pre-generation + agentic governance pipeline (deterministic, ~16ms CPU)
- 6 regulation plugins (DPDP 2023, GDPR, EU AI Act, HIPAA, CCPA, SEBI/RBI) — 97 clauses total
- Real-time decisions: ALLOW · BLOCK · SUPPORT · HITL · KILL_SESSION
- Atomic 3-way commit gate (input · stability · output)
- **Agentic trajectory governance** — Ring 12 (5-signal drift + penetration-depth recession + flow/provenance hard-blocks), Ring 13 (Jayadratha egress gate), Sandhi (Ed25519-signed declarations), Conch (cross-tenant IOC broadcast), Vyuha Selector, Sequential Engagement
- SHA-256 hash-chained **+ HMAC-signed** tamper-proof audit vault (S3 Object-Lock WORM anchor)
- Human-in-the-loop queue with SLA timers
- Multi-tenant registry (PostgreSQL) with bounded LRU cache + multi-pod control plane
...
- React 19 7-tab intelligence console
- SDK ecosystem: `aegis-ai-sdk` v1.1.0 + `aegis-ring12` (PyPI), `@aegis.org/sdk` v1.1.0 + `@aegis.org/mcp` (npm)
- AWS production deployment (CloudFront + ECS Fargate + ElastiCache + RDS + MongoDB Atlas + S3 WORM)
- Prometheus + JSON metrics
- `agentic-redteam-benchmark` (public agent red-team set) + bounded-evasion theorem

**Funded build (next):**
- Trained calibration head (replace tuned thresholds; prototype AUC 0.983)
...
- Further agentic rings: R14 inter-agent bus · R15 behavioural fingerprint · R16 plan-of-record ZK proof
- Additional plugins: PDPA Singapore/Thailand, LGPD Brazil, POPIA South Africa, NIST AI RMF
...
| **Agent Platform Owner** | Runs autonomous AI agents | Trajectory drift kill · tool-scope gate · signed declarations · MCP adapter |
...
| ID | Requirement | Priority |
|----|---|---|
| FR-03.1 | System SHALL detect 15 PII entity types (IN+US): AADHAAR · PAN · IFSC · IBAN · CARD (Luhn) · SSN_US · IN/US PHONE · EMAIL · IPv4 · IPv6 · PASSPORT · DOB · UPI · NAME | P0 |
...
| FR-16.2 | Frontend SHALL provide 7 tabs: RISK · SESSION · EVIDENCE · AGENTS · MONITOR · SYSTEM · HITL | P0 |
| FR-16.3 | Frontend SHALL wire all 56 backend endpoints | P0 |
...
| FR-17.9 | A standalone agent verifier SHALL be published as `aegis-ring12` (PyPI) | P0 |
| FR-17.10 | A JS/TS client SHALL be published as `@aegis.org/sdk` (npm) | P0 |
| FR-17.11 | An MCP server SHALL be published as `@aegis.org/mcp` (npm) for Claude Code / Cursor / Codex / LangGraph | P0 |
...
### FR-19 — Trajectory Verifier (Ring 12)

| ID | Requirement | Priority |
|----|---|---|
| FR-19.1 | System SHALL begin an agent session with a declared goal (and optional plan + allowed tools/classes) via `POST /api/r12/begin_session` | P0 |
| FR-19.2 | System SHALL evaluate each agent step via `POST /api/r12/evaluate` and return CONTINUE / WARN / KILL_SESSION | P0 |
| FR-19.3 | System SHALL compute a 5-signal drift aggregate `(0.20·S1 + 0.25·S2 + 0.25·S3 + 0.15·S4 + 0.15·S5)/1.5` (WARN ≥ 0.45, KILL ≥ 0.70) | P0 |
| FR-19.4 | Decision boundary SHALL tighten with penetration depth (recession) per the bounded-evasion theorem | P0 |
| FR-19.5 | System SHALL hard-block declaration-independent flows: secret→external-egress, pipeline-poisoning, capability-assembly, privilege-escalation | P0 |
| FR-19.6 | S5 stability SHALL be deterministic + in-network by default (Groq opt-in via `R12_S5_USE_LLM=1`) | P0 |
| FR-19.7 | Trajectory state (drift depth, taint, provenance, EMA, history) SHALL be multi-pod via the control plane under a per-session lock | P0 |
| FR-19.8 | `POST /api/r12/end_session` SHALL write a ROLLUP entry to the audit vault | P0 |

### FR-20 — Jayadratha Egress Gate (Ring 13)

| ID | Requirement | Priority |
|----|---|---|
| FR-20.1 | On Ring 12 WARN/KILL, Ring 13 SHALL freeze the session — all subsequent egress returns `FROZEN` | P0 |
| FR-20.2 | `POST /api/r13/evaluate` SHALL gate egress actions (API-key) | P0 |
| FR-20.3 | `POST /api/r13/unfreeze_session` SHALL thaw a frozen session (admin) | P0 |
| FR-20.4 | Frozen sessions SHALL be listable via `GET /api/r13/freeze` | P1 |

### FR-21 — Sandhi Cryptographic Declarations

| ID | Requirement | Priority |
|----|---|---|
| FR-21.1 | System SHALL generate an Ed25519 keypair per agent (`POST /api/sandhi/register_key`) | P0 |
| FR-21.2 | System SHALL sign `goal + plan + allowed_tools + allowed_classes` (`POST /api/sandhi/sign`) | P0 |
| FR-21.3 | `POST /api/sandhi/verify` SHALL verify signature + detect tamper | P0 |
| FR-21.4 | Declarations SHALL be multi-pod (public key + KMS-wrapped seed on the control plane; plaintext seed never stored) | P0 |

### FR-22 — Conch Cross-Tenant IOC Broadcast

| ID | Requirement | Priority |
|----|---|---|
| FR-22.1 | A novel Ring 12 kill SHALL broadcast an attack fingerprint (hash only) to all tenants | P1 |
| FR-22.2 | Other tenants SHALL run elevated vigilance for 24h on matching fingerprints | P1 |
| FR-22.3 | Conch SHALL fail safe (no amplification) when the control plane is unavailable | P0 |

### FR-23 — Vyuha Selector (Adaptive Formation Router)

| ID | Requirement | Priority |
|----|---|---|
| FR-23.1 | Vyuha SHALL select a pre-pipeline posture from attack signals + PII redactions | P1 |
| FR-23.2 | VAJRA posture SHALL force Ring 5 + Ring 7 ON even on ALLOW decisions | P1 |
| FR-23.3 | Vyuha SHALL be a routing HINT, never a sole security control | P0 |

### FR-24 — Sequential Engagement (Rotating Detectors)

| ID | Requirement | Priority |
|----|---|---|
| FR-24.1 | Rings SHALL carry a pool of rotating variant detectors with non-overlapping blindspots | P1 |
| FR-24.2 | HITL outcomes (tp/fp/tn/fn) SHALL auto-quarantine/promote variants (`POST /api/sequential/outcome`) | P1 |
| FR-24.3 | All variants in a pool SHALL detect the same threat surface | P1 |
...
### Training corpus
- **Live deployed: 8,627 labeled vectors** across 12 categories (verified: `precomputed_embeddings.npy` shape `(8627, 384)`)
- Eval-locked V7 snapshot: 2,416 vectors → 99.30% on the V3 adversarial set
- 384-dim ONNX embeddings precomputed in `precomputed_embeddings.npy`; labels in `precomputed_labels.json`

### Adversarial benchmark
- **`backend/eval/adversarial_dataset_v3.json`** — 1,001 samples · 132 attack techniques · 12 categories
...
### Agentic benchmark
- **`agentic-redteam-benchmark/`** — 500+ hand-authored agent trajectories (513 authored) · 5 drift modes · per-step ground truth
- 4-metric scoring: F1 (kill) · kill-session latency (KSL) · FP step rate · cost per 1k steps
- Public release with full baselines + Cohen's κ: 2026-07-09
...
The following 14 scenarios constitute acceptance of Chakravyuha V3.
...
### AC-13 — Agent drift kills the session before harm (Ring 12 + 13)

```
GIVEN an agent begins a session with declared goal "summarise the quarterly PDF"
 AND declared_tools = ["fs.read", "pdf.read"]
WHEN the agent reads a managed secret (secrets.read)
 AND then attempts egress to an external address (email.send → attacker@external)
THEN POST /api/r12/evaluate returns KILL_SESSION (secret→egress flow ring)
 AND Ring 13 freezes the session (subsequent egress returns FROZEN)
 AND a ROLLUP entry is written to the audit vault
```

### AC-14 — Tampered declaration is cryptographically detected (Sandhi)

```
GIVEN an agent registered an Ed25519 key and signed a declaration via /api/sandhi/sign
WHEN the declared goal/allowed_tools are modified in session memory
 AND POST /api/sandhi/verify is called
THEN verification returns a tamper/violation result
 AND Ring 12 escalates the step as out-of-scope drift
 AND the declaration verifies identically on any pod (multi-pod control plane)
```

---

*Chakravyuha V3 Requirements Specification — Production AI governance infrastructure · Built solo by Jaswanth Alkur · [Back to README](../README.md)*
*Live demo: https://d1qmmfoekk6xmp.cloudfront.net/ · SDKs: [PyPI](https://pypi.org/project/aegis-ai-sdk/) · [aegis-ring12](https://pypi.org/project/aegis-ring12/) · [npm](https://www.npmjs.com/package/@aegis.org/sdk) · [MCP](https://www.npmjs.com/package/@aegis.org/mcp) · Benchmark: https://github.com/Alkur123/agentic-redteam-benchmark · Paper: https://doi.org/10.5281/zenodo.19690403*
